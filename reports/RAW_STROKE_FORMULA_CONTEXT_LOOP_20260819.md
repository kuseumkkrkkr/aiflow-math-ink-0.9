...
- 같은 49식의 식 완전일치는 raw HWR `15/49`(30.61%), masked context `29/49`(59.18%), 기존 식 배치·guard 최종 `38/49`(77.55%), 후보 보존형 문맥 융합 `40/49`(81.63%), 숫자 문맥 gate 포함 최종 `41/49`(83.67%)였다.
- 문맥 융합은 `aiflow_0049`의 `/↔1`, `aiflow_0078`의 `5↔S`를 복원했다. 숫자 gate는 `aiflow_0052`의 `2 × 8 = i 6`을 `2 × 8 = 1 6`으로 복원했고 기존 정답 회귀는 0건이었다.
- 기존 HWR Top-5 식 oracle은 `41/49`였지만, 숫자 피연산자에만 허용한 기존·보조 Top-5 합집합 oracle은 `46/49`(93.88%)다. 남은 병목은 분할 1식, 합집합 후보 누락 2식, 합집합 안 선택 실패 5식이다.
...
- 연산자·관계기호·괄호는 바꾸지 않으며 문자 삽입·삭제와 산술 정답 계산을 하지 않는다.
...
`41/49`는 원시 stroke부터 자동 분할을 포함한 수치다. 기존 `85/95`는 검수된 glyph ownership을 입력으로 준 조건부 수치이므로 직접 비교하면 안 된다.

실제 raw runtime으로 유효 원시식 159개를 다시 실행했다. 시간순 뒤 49식은 `40→41`, ownership 96식은 `85→86`, 전체 prompt 159식은 `124→126`으로 올랐고 회귀는 0건이었다. 추가 복원은 `aiflow_0052`, `aiflow_0090`이다. 159식 모두 stroke exact-cover, 허용 후보 합집합 밖 문자 0, 삽입·삭제 0, 연산자·관계기호·괄호 변경 0, 제품 기본값 OFF를 확인했다. 최종 출력 SHA-256은 `876b9ec3e5b4f0445324aa9c1b2840cced25d1a737980f4fa4072e1868405868`이다.

## 남은 병목

- 분할 실패 1식: `x · \neq · y`의 3-stroke 관계기호를 2+1로 나눔.
- 기존·보조 Top-5 합집합 누락 2식: `aiflow_0046`, `aiflow_0064`.
- 합집합 안 선택 실패 5식: `aiflow_0004`, `aiflow_0056`, `aiflow_0058`, `aiflow_0061`, `aiflow_0072`.
