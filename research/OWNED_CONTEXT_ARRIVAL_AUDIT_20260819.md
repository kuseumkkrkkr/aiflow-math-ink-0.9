...
문자 형상 모델을 계속 키우는 대신, HWR가 낸 Top-5 후보를 수식 맥락으로 확정하는 레이어를 실제 추론 경계로 분리했다. 기존 r6을 새 수집 48식·176글자에 재학습 없이 적용하고 보수적 의미 가드를 연결해 Top-1을 65.91%에서 84.09%, 수식 exact를 31.25%에서 60.42%로 높였고 r6 정답 회귀는 0건이었다.

확장 데이터로 다시 학습한 모델은 직접 수집과 CROHME 모두 r6보다 약해 교체하지 않았다. 사후 앙상블과 등호 없는 산술 강제 규칙도 외부 전이 회귀가 확인되어 넣지 않았다. 제품 호출 경계는 r6, 정확한 산술 등식 가드, 기존 괄호 잠금 보완 가드로 고정하되 opt-in shadow로 유지한다. 이번 새 수집분에는 정답 `|`와 `o`가 없어 상용 정확도 gate를 통과했다고 판정할 수 없다.
...
r6은 각 글자를 문맥으로 재정렬하지만 수식 전체가 실제로 성립하는지는 보지 않았다. 정답을 읽지 않고 Top-5 후보 lattice만 사용하는 두 가드를 r6 뒤에 추가했다.
...
- 두 가드 모두 beam 64이며 HWR Top-5 밖 후보, 삭제, grouping 변경은 금지한다.

| 범위 | 지표 | HWR | r6 | r6 + 의미 가드 | 의미 가드 추가 개선/회귀 |
|---|---|---:|---:|---:|---:|
| 전체 95식 | Top-1 | 70.03% | 86.30% | **87.34%** | 4/0 |
| 전체 95식 | 수식 exact | 32.63% | 60.00% | **64.21%** | 4식/0식 |
| 신규 48식 | Top-1 | 65.91% | 82.95% | **84.09%** | 2/0 |
| 신규 48식 | 수식 exact | 31.25% | 56.25% | **60.42%** | 2식/0식 |
| 신규 writer 44식 | Top-1 | 65.12% | 82.56% | **83.72%** | 2/0 |
| 신규 writer 44식 | 수식 exact | 25.00% | 52.27% | **56.82%** | 2식/0식 |
| CROHME 984식 | Top-1 | 69.50% | 77.82% | **78.00%** | 17/0 |
| CROHME 984식 | 수식 exact | 15.85% | 27.95% | **28.46%** | 5식/0식 |

전체 95식 strict micro는 79.17%→80.21%, strict macro는 77.19%→77.68%로 올랐다. CROHME strict micro 63.15%, strict macro 52.01%는 변하지 않았다. 의미 가드가 r6 위에 새로 만든 회귀는 직접·CROHME 모두 0건이다.

산술 등식 가드는 직접 3글자를 복원했고 CROHME에서는 바꾸지 않았다. 괄호 가드는 직접 `f ( y ) = 1`의 여는 괄호 1개와 CROHME 17글자를 복원했다. 더 공격적인 등식 가드는 `0×q=0→0×7=0`, `1b÷8=2→16÷8=2`, `ξ0÷5=6→30÷5=6`, `ℏ+0=5→5+0=5`처럼 성립하는 기호식을 숫자로 오염시켜 기각했다. 등호 없는 순수 산술 강제와 r6 결과를 다시 넣는 2회 추론도 각각 회귀와 미미한 효과 때문에 기각했다.

- 최종 평가: `D:\AIFlow-Workspace\PrivateData\math-ink-data-collector\derived\context-candidates-20260819-r3\r6_semantic_eval_r6.json`
- 평가 SHA-256: `6d5229895719be4a1e69c6850e98dc6c1c97edb052f4aa000fa4417584940d3a`
- 정답 없는 런타임 출력: `D:\AIFlow-Workspace\PrivateData\math-ink-data-collector\derived\context-candidates-20260819-r3\runtime-r6-semantic-r6.jsonl.gz`
- 런타임 출력 SHA-256: `9fa6ddf55eabbd8d3108de9bc7780cb4edf1ac2f6d548159b6f1f4b2ffd25399`
