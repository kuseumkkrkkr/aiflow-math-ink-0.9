#!/usr/bin/env python3
"""Evaluate straight equality-slot rescue on direct and CROHME scopes."""
...
from straight_equality_slot_rescue_v1 import (
    ...
    _self_test as rescue_self_test, _write, apply_straight_equality_slot_rescue,
)


SCHEMA = "aiflow-straight-equality-slot-rescue-evaluation/v1"
...
def main() -> int:
    ...
    if not all(path.is_file() for path in paths[:4]):
        parser.error("one or more straight-equality inputs are missing")
    ...
    if report_path.exists() or config_path.exists() or any(
        path.exists() for path in runtime_paths
    ):
        parser.error("refusing to overwrite straight-equality evidence")
    ...
    direct_rows, direct_audit = apply_straight_equality_slot_rescue(
        direct_baseline, direct_top20, DEFAULT_CONFIGURATION,
    )
    crohme_rows, crohme_audit = apply_straight_equality_slot_rescue(
    ...
    contract_passed = bool(
        ...
        and direct_audit["arithmetic_evaluation"] is False
        and crohme_audit["arithmetic_evaluation"] is False
    )
    shadow_admitted = bool(
        direct["glyph_improved"] == 2
        and direct["glyph_regressed"] == 0
        and direct["formula_improved"] == 1
        and direct["formula_regressed"] == 0
        and crohme["glyph_regressed"] == crohme["formula_regressed"] == 0
        and contract_passed
    )
    gate = {
        "shadow_runtime_admitted": shadow_admitted,
        "product_default_admitted": False,
        "criteria": {
            "direct_two_glyph_improvements": direct["glyph_improved"] == 2,
            "direct_one_formula_improvement": direct["formula_improved"] == 1,
            "direct_regressions_zero": (
                direct["glyph_regressed"] == direct["formula_regressed"] == 0
            ),
            "crohme_repeated_diagnostic_regressions_zero": (
                crohme["glyph_regressed"] == crohme["formula_regressed"] == 0
            ),
            "candidate_layout_and_arithmetic_contract_passed": contract_passed,
        },
        "promotion_requirement": (
            "untouched project-owned writer-disjoint and formula-disjoint "
            "commercial acceptance with positive straight-equality cases"
        ),
    }
    ...
    report = {
        ...
        "limits": [
            ...
            "CROHME contains no eligible positive application in this gate",
            "fresh commercial straight-equality acceptance remains required",
        ],
    }
