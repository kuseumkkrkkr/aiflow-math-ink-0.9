...
def audit(run_dir: Path) -> dict[str, Any]:
    ...
    for domain, input_key in DOMAIN_INPUTS.items():
        if not inputs.get(input_key):
            continue
        path = Path(inputs[input_key]).resolve()
        source_record, metadata = _source_audit(path, inputs.get(f"{input_key}_sha256"))
        with np.load(path, allow_pickle=False) as payload:
            features = np.asarray(payload["features"], dtype=np.float32)
            labels = np.asarray(payload["labels"], dtype=np.int64)
    ...
    checkpoint = Path(inputs["checkpoint"]).resolve()
    if _sha256(checkpoint) != inputs.get("checkpoint_sha256"):
        raise ValueError("checkpoint hash mismatch")
    lineage = _checkpoint_lineage(checkpoint, source_manifests[0])
    evaluation_boundary = _evaluation_boundary(
        source_manifests[0], "project_owned_cleanroom" in source_records,
    )
    ...
    gates = {
        ...
        "augmentation_label_safety_top5_90pct": bool(top5_values and minimum_top5 >= 0.90),
        "training_or_augmentation_admission_allowed": not evaluation_boundary["evaluation_only_parent_lineage_used"],
    ...
    return {
        ...
        "decision": (
            ...
            if structural else "REJECT_STRUCTURAL_FAILURE"
        ),
