#!/usr/bin/env python3
"""Evaluate the paired straight-one numeric operand slot rescue."""
...
SCHEMA = "aiflow-dual-straight-one-slot-rescue-evaluation/v1"
CONFIG_SCHEMA = "aiflow-dual-straight-one-slot-rescue-runtime-config/v1"
...
def main() -> int:
    ...
    if not all(path.is_file() for path in paths[:4]):
        parser.error("one or more dual-straight-one inputs are missing")
    ...
    if report_path.exists() or config_path.exists() or any(
        ...
        parser.error("refusing to overwrite dual-straight-one evidence")
    ...
    direct_rows, direct_audit = apply_dual_straight_one_slot_rescue(
        direct_baseline, direct_top20, DEFAULT_CONFIGURATION,
    )
    crohme_rows, crohme_audit = apply_dual_straight_one_slot_rescue(
