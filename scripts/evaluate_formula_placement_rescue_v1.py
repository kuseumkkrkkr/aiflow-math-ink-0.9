#!/usr/bin/env python3
"""Evaluate the composed conservative formula placement rescue layer."""
...
SCHEMA = "aiflow-formula-placement-rescue-evaluation/v1"
...
def main() -> int:
    ...
    if not all(path.is_file() for path in paths[:4]):
        parser.error("one or more formula placement inputs are missing")
    ...
    if report_path.exists() or config_path.exists() or any(
        ...
        parser.error("refusing to overwrite formula placement evidence")
    ...
    direct_rows, direct_audit = apply_formula_placement_rescue(
        direct_baseline, direct_top20, DEFAULT_CONFIGURATION,
    )
    crohme_rows, crohme_audit = apply_formula_placement_rescue(
    ...
    invariant_keys = (
        "tokens_outside_candidate_union", "inserted_or_deleted_glyphs",
        "glyph_order_mutations", "grouping_mutations", "layout_mutations",
    )
    ...
    stage_replay_passed = bool(
        direct_audit["stage_changed_glyphs"] == {
            "unmatched_fence_operand": 1,
            "operator_cross_slot": 1,
            "dual_straight_one_slot": 2,
        }
        and crohme_audit["stage_changed_glyphs"]
        == {name: 0 for name in STAGE_ORDER}
    )
    shadow_runtime_admitted = bool(
        direct["formula_improved"] == 3
    ...
    gate = {
        ...
        "criteria": {
            "direct_three_formula_improvements": direct["formula_improved"] == 3,
            ...
            "stage_replay_passed": stage_replay_passed,
        },
    ...
    report = {
        ...
        "limits": [
            ...
            "the candidate contract uses research-only Top-20 retrieval",
    ...
    print(json.dumps({
        ...
        "stage_changes": {
            "direct": direct_audit["stage_changed_glyphs"],
            "crohme": crohme_audit["stage_changed_glyphs"],
        },
