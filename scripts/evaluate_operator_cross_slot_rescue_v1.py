#!/usr/bin/env python3
"""Evaluate the digit-bot-digit operator-cross placement rescue."""
...
SCHEMA = "aiflow-operator-cross-slot-rescue-evaluation/v1"
CONFIG_SCHEMA = "aiflow-operator-cross-slot-rescue-runtime-config/v1"
...
def main() -> int:
    ...
    if not all(path.is_file() for path in paths[:4]):
        parser.error("one or more operator-cross inputs are missing")
    ...
    if report_path.exists() or config_path.exists() or any(
        ...
        parser.error("refusing to overwrite operator-cross evidence")
    ...
    direct_rows, direct_audit = apply_operator_cross_slot_rescue(
        direct_baseline, direct_top20, DEFAULT_CONFIGURATION,
    )
    crohme_rows, crohme_audit = apply_operator_cross_slot_rescue(
