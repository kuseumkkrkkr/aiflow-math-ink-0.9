...
DEFAULT_OUTPUT = ROOT / "artifacts/sparse_writer_style_scorer_v10_writers064_095_outer_r1"
DEFAULT_CACHE = ROOT / "artifacts/sparse_writer_style_scorer_v10_writers064_095_outer_r1_cache.npz"
...
def _outer_episode(writer: int, size: int, data: dict, stats: dict, groups: list[list[int]], v10):
    # _episode_indices uses labels only for calibration selection. Query labels
    # are not read by this builder or by the prediction API below.
    calibration, query = v10._episode_indices(writer, size, data["labels"], data["writers"], data["splits"])
    context = v10._episode_context(calibration, data, stats)
    records = []
    features = []
    for index in query.tolist():
        values, candidates, ranks = v10._row_features(index, context, data, stats, groups)
    ...
    for index in calibration.tolist():
        full_support[int(data["labels"][index])] += 1
    self_features = []
    self_records = []
    for index in calibration.tolist():
        local_context = v10._episode_context(calibration, data, stats, index, full_support)
        values, candidates, ranks = v10._row_features(index, local_context, data, stats, groups)
        for feature, candidate, rank in zip(values, candidates, ranks, strict=True):
...
def _predict_without_query_truth(episode: dict, data: dict, model, mean, scale, threshold: float, v10):
    np = v10.np
    ...
    preliminary = v10.apply_predictions(data["logits"][calibration], local_options, self_probability, threshold, True)
    calibration_truth = data["labels"][calibration]
    calibration_baseline = np.argmax(data["logits"][calibration], axis=1)
    evidence = {}
    allowed = set()
    ...
    self_prediction = v10.apply_predictions(data["logits"][calibration], self_options, self_prob, threshold, bool(allowed))
    ...
    prediction = v10.apply_predictions(data["logits"][query], query_options, query_probability, threshold, enabled)
    return prediction, {"enabled": enabled, "allowed_candidates": sorted(allowed), "candidate_evidence": evidence}


def _score_episode(episode: dict, prediction, state: dict, data: dict, writer_names: dict[int, str], v10):
    np = v10.np
    query = episode["query"]
    logits = data["logits"][query]
    truth = data["labels"][query]  # scoring-only access after prediction is frozen
...
def _open(receipt: dict, output: Path, cache: Path) -> int:
    ...
    data = {
        "labels": labels, "writers": (local_writers + 64).astype(np.int16), "splits": splits,
    ...
    np.savez_compressed(cache, labels=data["labels"], writers=data["writers"], local_writers=local_writers, splits=splits,
                        embeddings=data["embeddings"], logits=data["logits"], bank_sha256=np.asarray(sha256(BANK / "synthetic_writer_style_bank_v5_r4.npz")),
    ...
    episodes = [_outer_episode(writer, size, data, stats, groups, v10) for writer in EXPECTED_GLOBAL_WRITERS for size in SUPPORT_SIZES]
    rows = []
    trace = []
    for episode in episodes:
        prediction, state = _predict_without_query_truth(episode, data, model, mean, scale, float(config["threshold"]), v10)
        row, changed = _score_episode(episode, prediction, state, data, writer_names, v10)
    ...
    gates = {
        ...
        "writer_namespace_exact_064_095": set(data["writers"].tolist()) == set(EXPECTED_GLOBAL_WRITERS),
...
def main() -> int:
    ...
    return _open(actual, args.output, args.cache)
