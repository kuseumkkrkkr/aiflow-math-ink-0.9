...
2026-08-19 수집분을 포함한 ownership 96식 중 고정 372-class HWR에 없는 `t`가 든 한 식을 통째로 제외하고 95식·387글자를 감사했다. 기존 r6을 새 수집 48식에 재학습 없이 적용한 뒤, Top-5 lattice에서 정확한 산술 등식, 누락된 괄호 짝, 수평 값 사이의 유일한 중위 연산자만 복원하는 결정적 의미 가드를 연결했다. 등식 v2는 현재 선택열의 비산술 기호가 한 슬롯이고 정확한 등식을 만드는 산술 후보가 하나뿐일 때만 추가 확정한다. HWR Top-1 65.91%는 최종 85.80%, 수식 exact 31.25%는 66.67%로 올랐고 r6 정답 회귀는 0건이었다. 반면 95식 재학습 challenger, HWR 문맥 노이즈 재학습, 반복 추론, 등호 없는 산술 강제와 다중 기호 숫자화는 효과 부족 또는 안전성 문제로 기각했다.

실제 추론은 [scripts/finalize_formula_context_v1.py](scripts/finalize_formula_context_v1.py)가 담당한다. 정답 `label` 없이 HWR Top-5·확률·수식 순서·공간관계만 받고, 후보 추가·글자 삭제·stroke regrouping 없이 후보 하나를 확정한다. 길이 1 입력은 호환 Top-1을 유지하되 수식 맥락이 없음을 `ambiguous_no_formula_context`로 명시한다. 전체 직접수집 Top-1은 88.63%, 수식 exact는 69.47%이며 의미 가드는 r6 위에서 9개를 추가 복원하고 회귀는 0건이었다. CROHME도 r6 대비 Top-1 77.82%→78.09%, 수식 exact 27.95%→28.56%, 추가 개선/회귀 26/0이었다. r6+의미 가드는 선택 경로지만 아직 opt-in shadow 상태다. 새 수집분에는 `|`와 `o` 정답이 없어 상용 정확도 gate는 유지하며, 전체 감사는 [research/OWNED_CONTEXT_ARRIVAL_AUDIT_20260819.md](research/OWNED_CONTEXT_ARRIVAL_AUDIT_20260819.md)에 기록했다.
